...
1. **شروع کار با ربات**: 
   - کاربر دستور `/start` را به ربات (مثلاً `@MyAudioLibraryBot`) ارسال می‌کند.
   - ثبت‌نام کاربر انجام شده و دسته‌بندی‌ها به او نمایش داده می‌شود.

2. **دریافت کارنامه فعالیت**:
   - دستور: `/dailyreport` یا `/myreport`
   - با ارسال این دستور، ربات گزارشی شامل تعداد کل فایل‌های دریافتی، و جزئیات هر دسته (دریافت شده، باقی‌مانده و لینک دریافت آیتم بعدی) را ارسال می‌کند.

3. **مشاهده پادکست‌های جدید**:
   - دستور: `/whatsnew` یا `تازه ها`
   - با ارسال این دستور، ربات چک می‌کند از آخرین باری که کاربر فایلی دریافت کرده، چه فایل‌های جدیدی به سیستم اضافه شده (فقط دسته‌هایی که محتوای جدید دارند نمایش داده می‌شوند).

4. **دریافت محتوای یک دسته خاص**:
   - دستور: `/category{ID}` (مثلاً `/category15`)
   - با کلیک روی این لینک‌ها (که در کارنامه و بخش تازه‌ها تولید می‌شوند)، بلافاصله فایل بعدیِ همان دسته برای کاربر ارسال می‌شود.

### ۵. رتبه‌بندی کاربران (`/topusers`) — ویژه ادمین
ادمین می‌تواند با ارسال دستور `/topusers` در ربات Reader، **۱۰ کاربر برتر** را بر اساس تعداد کل فایل‌های دریافتی مشاهده کند.

### ۶. ارسال پیام به کاربر خاص (`/sendmsg`) — ویژه ادمین
