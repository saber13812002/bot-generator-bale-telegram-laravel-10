...
## 📊 امکانات جدید (گزارش و ادمین)

### ۱. گزارش پیشرفت کاربر (`/dailyreport`)
کاربر می‌تواند در ربات Reader با ارسال دستور `/dailyreport` یا `/myreport`، آمار دقیقی از فایل‌های دریافت‌شده مشاهده کند:
- نمایش **تعداد کل فایل‌های صوتی دریافتی** کاربر.
- نمایش **فایل‌های دریافت‌شده و باقیمانده** به تفکیک هر دسته‌بندی (Category).

### ۲. رتبه‌بندی کاربران (`/topusers`) — ویژه ادمین
ادمین می‌تواند با ارسال دستور `/topusers` در ربات Reader، **۱۰ کاربر برتر** را بر اساس تعداد کل فایل‌های دریافتی مشاهده کند.

### ۳. ارسال پیام به کاربر خاص (`/sendmsg`) — ویژه ادمین
ادمین می‌تواند با استفاده از دستور `/sendmsg {شناسه_کاربر} {متن پیام}`، به صورت مستقیم برای یکی از کاربران پیام ارسال کند.
- **مثال**: `/sendmsg 123 سلام! شما در رتبه اول دریافت فایل‌های صوتی هستید.`
- ربات یک پیام تأیید ارسال به ادمین برمی‌گرداند.
