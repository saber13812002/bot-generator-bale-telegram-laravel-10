<?php
return [
    'word by word'                                                                => 'کلمه به کلمه',
    'ayah after ayah'                                                             => 'آیه به آیه',
    'List of 114 Surahs'                                                          => 'فهرست 114 سوره',
    'List of 30 Juz'                                                              => 'فهرست 30 جزء',
    'this bot support 2 methods'                                                  => 'دو جور مطالعه قرآن داریم',
    'one of them'                                                                 => 'یکی ',
    'another method is'                                                           => 'روش دیگه ',
    'start from second button'                                                    => 'از دکمه دوم شروع کنید',
    'third button'                                                                => 'دکمه سوم',
    'forth button'                                                                => 'دکمه چهارم',
    'next aya'                                                                    => 'آیه بعدی',
    'previous aya'                                                                => 'آیه قبلی',
    'next surah'                                                                  => 'سوره بعدی',
    'previous surah'                                                              => 'سوره قبلی',
    'bot cant recognized your command'                                            => 'دستور نا مشخص',
    'return to command list'                                                      => 'بازگشت به فهرست',
    'Juz'                                                                         => 'جزء',
    'and'                                                                         => 'و',
    'this command not recognized'                                                 => 'دستور شما منجر به هیچ کاری نشد',
    
    // Book Pixel Bot translations
    'book_pixel_welcome'                                                          => '📚 خوش آمدید به ربات یک پیکسل کتاب!\n\nاین ربات به شما امکان به اشتراک‌گذاری صفحات کتاب را می‌دهد.',
    'book_pixel_ask_book_name'                                                    => 'لطفاً نام کتاب یا ISBN/شابک را ارسال کنید:',
    'book_pixel_ask_isbn'                                                         => 'لطفاً ISBN کتاب را ارسال کنید (اختیاری):',
    'book_pixel_ask_shabak'                                                       => 'لطفاً شابک کتاب را ارسال کنید (اختیاری):',
    'book_pixel_ask_cover'                                                        => 'لطفاً عکس جلد کتاب را ارسال کنید:',
    'book_pixel_ask_page_number'                                                  => 'لطفاً شماره صفحه را ارسال کنید:',
    'book_pixel_ask_scan'                                                         => 'لطفاً اسکن صفحه کتاب را ارسال کنید:',
    'book_pixel_ask_voice'                                                        => 'لطفاً وویس صفحه را ارسال کنید (اختیاری):',
    'book_pixel_book_not_found'                                                    => 'کتاب یافت نشد. لطفاً دوباره تلاش کنید.',
    'book_pixel_book_found'                                                        => 'کتاب "{name}" یافت شد.',
    'book_pixel_book_created'                                                      => 'کتاب "{name}" با موفقیت ثبت شد.',
    'book_pixel_scan_sent_for_approval'                                            => '✅ اسکن صفحه برای تایید ارسال شد. پس از تایید، در صف انتشار قرار می‌گیرد.',
    'book_pixel_voice_sent'                                                        => '✅ وویس با موفقیت ارسال شد.',
    'book_pixel_score_message'                                                     => '📊 امتیاز شما:\n\n🎯 مجموع امتیازات: {points}\n📄 تعداد اسکن‌ها: {scans}\n🎤 تعداد وویس‌ها: {voices}',
    'book_pixel_no_score_yet'                                                      => 'شما هنوز امتیازی ندارید. با ارسال اسکن و وویس امتیاز کسب کنید!',
    'book_pixel_invalid_page_number'                                               => 'شماره صفحه نامعتبر است. لطفاً یک عدد صحیح ارسال کنید.',
    'book_pixel_missing_data'                                                      => 'اطلاعات ناقص است. لطفاً دوباره شروع کنید.',
    'book_pixel_unexpected_photo'                                                   => 'لطفاً ابتدا دستور /start را ارسال کنید.',
    'book_pixel_no_scan_for_voice'                                                 => 'ابتدا باید یک اسکن صفحه ارسال کنید.',
    'book_pixel_please_send_cover'                                                  => 'لطفاً عکس جلد کتاب را ارسال کنید.',
    'book_pixel_please_send_scan'                                                   => 'لطفاً اسکن صفحه کتاب را ارسال کنید.',
    'book_pixel_error_saving_cover'                                                 => '❌ خطا در ذخیره عکس جلد. لطفاً دوباره تلاش کنید.',
    'book_pixel_ask_isbn_first'                                                     => 'لطفاً شابک/ISBN کتاب را ارسال کنید:',
    'book_pixel_book_ready_no_cover'                                                => 'کتاب آماده است. می‌توانید عکس جلد ارسال کنید یا رد کنید:',
    'book_pixel_photo_received'                                                     => '✅ عکس دریافت شد.',
    'book_pixel_help'                                                               => '📚 راهنمای ربات یک پیکسل کتاب:\n\n' .
        '/start - شروع\n' .
        '/help - راهنما\n' .
        '/cover - ارسال عکس جلد\n' .
        '/search <query> - جستجوی کتاب\n' .
        '/drafts - لیست پیش‌نویس‌ها\n' .
        '/cancel - لغو فرآیند\n' .
        '/skip - رد کردن فیلد اختیاری\n' .
        '/score - نمایش امتیاز\n' .
        '/stats - آمار (فقط در گروه)',
    'book_pixel_no_books_found'                                                     => 'کتابی یافت نشد.',
    'book_pixel_search_results'                                                     => 'نتایج جستجو:',
    'book_pixel_no_drafts'                                                          => 'پیش‌نویسی وجود ندارد.',
    'book_pixel_drafts_list'                                                        => 'لیست پیش‌نویس‌ها:',
    'book_pixel_cancelled'                                                          => '✅ فرآیند لغو شد.',
    'book_pixel_cannot_skip'                                                        => '❌ نمی‌توانید این مرحله را رد کنید.',
    'book_pixel_nothing_to_skip'                                                    => 'چیزی برای رد کردن وجود ندارد.',
    'book_pixel_stats_only_in_group'                                                => 'دستور /stats فقط در گروه کار می‌کند.',
    'book_pixel_statistics'                                                         => 'آمار ربات',
    'skip'                                                                          => 'رد کردن',
    'send_cover'                                                                    => 'ارسال جلد',
    'cover'                                                                         => 'جلد',
    'scan'                                                                          => 'اسکن',
    'voice'                                                                         => 'وویس',
    'queue'                                                                         => 'صف',
    'pending_approval'                                                              => 'در انتظار تایید',
    'pending_publishing'                                                            => 'در انتظار انتشار',
    'in_publishing_queue'                                                           => 'در صف انتشار',
    'completed'                                                                     => 'تکمیل شده',
    'today'                                                                         => 'امروز',
    'last_week'                                                                     => 'هفته گذشته',
    'last_month'                                                                    => 'ماه گذشته',
    'last_year'                                                                     => 'سال گذشته',
    'publishing_channels'                                                           => 'کانال‌های انتشار',
    'new'                                                                           => 'جدید',
    'new_users_today'                                                               => 'کاربران جدید امروز',
    'new_books_today'                                                               => 'کتاب‌های جدید امروز',
    'drafts'                                                                        => 'پیش‌نویس‌ها',
    'missions'                                                                      => 'ماموریت‌ها',
    'pending'                                                                       => 'در انتظار',
    'assigned'                                                                      => 'اختصاص داده شده',
    'this is not correct bot token'                                               => 'این یک توکن تلگرام یا بله نیست',
    'send your token to turn on your bot'                                         => 'برای ساخت روبات توکن را بفرستید',
    'i cant configure your bot'                                                   => 'من نمی توانم ربات شما را پیکربندی کنم',
    'please call admin by this account'                                           => 'وب هوک ست نشد. با ادمین تماس بگیرید',
    'your bots'                                                                   => 'روبات شما',
    'created'                                                                     => 'ساخته شد.',
    'Your bots:'                                                                  => 'ساخته شد.',
    'your bot created as well'                                                    => [
        '' => 'روبات شما ساخته شده است ',
    ],
    'bale'                                                                        => 'بله',
    'telegram'                                                                    => 'تلگرام',
    'please invite your friends via your bot link or bot username'                => 'لطفا دوستان خود را از طریق لینک ربات یا نام کاربری ربات خود دعوت کنید',
    'Your :bot bot created as well'                                               => 'روبات :bot شما ساخته شد.',
    'If you need :bot bot please send another token'                              => 'اگر به ربات :bot نیاز دارید، لطفا توکن دیگری را ارسال کنید',
    'Bot creation failed please contact us : '                                    => 'ایجاد ربات انجام نشد لطفا با ما تماس بگیرید:',
    'surah number:'                                                               => 'سوره شماره ',
    'to'                                                                          => 'تا',
    'You can start your activities'                                               => 'می توانید فعالیت های خود را شروع کنید.',
    'Your request accepted as well'                                               => 'روبات شما ساخته شده است',
    'An error occurred when admin want to approve your request'                   => 'هنگامی که مدیر می خواهد درخواست شما را تأیید کند، خطایی روی داد.تایید یک ادمین برای یک کاربر با خطا مواجه شد.',
    'Please determine the minimum wind speed for bot to send you desired alert'   => 'لطفاً حداقل سرعت باد را برای ربات تعیین کنید تا هشدار مورد نظر شما را ارسال کند.حدقل سرعت باد رو با ارسال یک عدد کوچکتر از 20 تعیین کنید که پیش بینی رو براتون بفرستم',
    'previous'                                                                    => 'قبلی',
    'next'                                                                        => 'بعدی',
    'please wait'                                                                 => 'لطفا صبر کنید ...',
    'processing your request'                                                     => '⏳ در حال پردازش درخواست شما...',
    'your chat id'                                                                => 'شناسه چت شما:',
    'forwarded message info'                                                       => '📤 اطلاعات پیام فوروارد شده:',
    'forwarded from chat'                                                          => '📢 فوروارد از :type (شناسه: :id):title',
    'forwarded from user'                                                          => '👤 فوروارد از کاربر (شناسه: :id):name:username',
    'forward signature'                                                            => '✍️ امضای فوروارد',
    'forward date'                                                                 => '📅 تاریخ فوروارد',
    'message sender'                                                               => '👤 ارسال‌کننده پیام (شناسه: :id):name:username',
    'current chat'                                                                 => '💬 چت فعلی: :type (شناسه: :id):title',
    'chat type channel'                                                            => 'کانال',
    'chat type group'                                                               => 'گروه',
    'chat type supergroup'                                                          => 'سوپرگروه',
    'chat type private'                                                             => 'خصوصی',
    'forwarded from group unknown'                                                  => '📢 فوروارد از گروه (اطلاعات گروه در دسترس نیست)',
    'forwarded by user'                                                             => '👤 فوروارد شده توسط کاربر (شناسه: :id):name:username',
    'forwarded group info note'                                                      => '💡 نکته: برای مشاهده اطلاعات کامل گروه (شناسه و نام)، ربات باید عضو گروه باشد.',
    'sending to blog api'                                                         => 'ارسال درخواست به وب سرویس وبلاگ ...',
    'sending to blog api but nothing returned'                                    => 'ارسال درخواست به وب سرویس وبلاگ انجام شد اما چیزی برنگشت...',
    'blog_messenger_not_found'                                                    => 'تنظیمات کانال شما یافت نشد. لطفاً با ادمین تماس بگیرید.',
    'blog_start_message'                                                         => 'پیام یا فایل بفرستید تا در کانال‌های شما (تلگرام، بله، ایتا) منتشر شود.',
    'blog_no_content'                                                             => 'لطفاً یک متن یا فایل ارسال کنید.',
    'your ranking'                                                                => 'رتبه شما',
    'change reciter'                                                              => 'تغییر قاری',
    'disable enable reciter'                                                      => 'غیرفعال/فعال کردن تلاوت کننده',
    'enable reciter'                                                              => 'فعال کردن قاری',
    'disable reciter'                                                             => 'قاری را غیرفعال کنید',
    'enabled'                                                                     => 'فعال شد',
    'disabled'                                                                    => 'غیرفعال',
    'please enable mp3 by'                                                        => 'برای فعال شدن ارسال فایل صوتی آیات:',
    'please disable mp3 by'                                                       => 'برای غیرفعال شدن ارسال فایل صوتی آیات:',
    'this reciter :reciter selected'                                              => 'این قاری :reciter انتخاب شد',
    'reciter :reciter'                                                            => 'تغییر دادن؟ تلاوت کننده به :reciter',
    'parhizgar'                                                                   => 'پرهیزگار',
    'alafasy'                                                                     => 'العفاسی',
    // ── بهبودها: قاری‌ها و جستجو ─────────────────────────────────────────
    'search'                                                                      => 'جستجو',
    'search prompt'                                                               => '🔍 عبارت مورد نظر خود را برای جستجو در قرآن ارسال کنید.',
    'next page'                                                                   => 'صفحه بعدی',
    'surah number :sura'                                                          => 'سوره :sura',
    'ayah number :aya'                                                            => 'آیه :aya',
    'n results found'                                                             => ':count مورد یافت شد.',
    'no results found'                                                            => 'هیچ موردی به عنوان نتیجه جستجوی شما یافت نشد.',
    'current reciter :reciter'                                                    => '🎧 قاری فعلی: :reciter',
    'select reciter'                                                              => 'انتخاب قاری',
    'reciter not found'                                                           => 'قاری مورد نظر یافت نشد. لطفاً از دکمه‌ها انتخاب کنید.',
    'settings mp3 status'                                                         => 'تلاوت صوتی',
    'your quran readings analysis report'                                         => 'گزارش تحلیل قرائت قرآن شما',
    'send mp3 for selected reciter'                                               => 'ارسال mp3 برای قاری منتخب',
    'disable sending mp3 for every ayah'                                          => 'غیرفعال کردن ارسال mp3 برای هر آیه',
    'view available translations'                                                 => 'مشاهده ترجمه‌های موجود',
    'your today readings is zero'                                                 => 'خواندن امروز شما صفر است',
    'your ranking in last 30 days is'                                             => 'رتبه شما در 30 روز گذشته است',
    'your todays usage of this bot'                                               => 'استفاده امروزی شما از این ربات',
    'ayah'                                                                        => 'آیه',
    'which compared to the previous day'                                          => 'که نسبت به روز قبل',
    'unique users of todays statistics'                                           => 'کاربران منحصر به فرد آمار امروز',
    'number of ayah in last week by all users'                                    => 'تعداد آیه در هفته گذشته توسط همه کاربران',
    'unique users in last week'                                                   => 'کاربران منحصر به فرد در هفته گذشته',
    'number of ayah in last month by all users'                                   => 'تعداد آیه در ماه گذشته توسط همه کاربران',
    'unique users in last month'                                                  => 'کاربران منحصر به فرد در ماه گذشته',
    'number of ayah in last year by all users'                                    => 'تعداد آیه در سال گذشته توسط همه کاربران',
    'unique users in last year'                                                   => 'کاربران منحصر به فرد در سال گذشته',
    'please help us to promote this bot to other people'                          => 'لطفا به ما کمک کنید تا این ربات را به دیگران معرفی کنیم',
    'command list is'                                                             => 'لیست دستورات:',
    'you have advantage'                                                          => 'شما پیشرفت داشته اید',
    'your readings less that yesterday activity'                                  => 'خواندن شما کمتر از فعالیت دیروز است',
    'today usage of this bot'                                                     => 'کل استفاده امروز از این ربات',
    'for direct access to sura and ayah'                                          => 'برای دسترسی مستقیم به سوره و آیات',
    'for example if you want to go sure 2 ayah 3'                                 => 'برای مثال اگر می خواهید بروید مطمئن شوید 2 آیه 3',
    'for search please type your phrase after double slash'                       => [
        ' like this' => 'برای جستجو لطفا عبارت خود را بعد از دو اسلش مانند این تایپ کنید:',
    ],
    'please enable it by'                                                         => 'لطفا آن را فعال کنید',
    'please disable it by'                                                        => 'لطفا آن را غیرفعال کنید',
    'to disable'                                                                  => 'برای غیر فعال کردن',
    'to enable transliteration'                                                   => 'برای فعال کردن نویسه‌گردانی',
    'trans_2'                                                                     => 'به ترجمه انصاریان تغییر کرد.',
    'trans_3'                                                                     => 'به ترجمه آیتی تغییر کرد.',
    'please change it to trans_2'                                                 => 'به کمک دستور زیر آن را به ترجمه انصاریان تغییر دهید.',
    'please change it to trans_3'                                                 => 'به کمک دستور زیر آن را به ترجمه آیتی تغییر دهید.',
    'no translations found for language'                                          => 'ترجمه‌ای برای زبان',
    'available translations for language'                                          => 'ترجمه‌های موجود برای زبان',
    'current default'                                                              => 'پیش‌فرض فعلی',
    'select translation by clicking button'                                        => 'برای تغییر ترجمه، روی دکمه مورد نظر کلیک کنید.',
    'translation changed to'                                                       => 'ترجمه به',
    'translation set to'                                                           => 'ترجمه تنظیم شد به',
    'current language settings kept'                                               => 'تنظیمات زبان فعلی حفظ شد',
    'would you like to use translation in'                                         => 'آیا می‌خواهید از ترجمه به زبان',
    'yes use'                                                                      => 'بله، استفاده کنم',
    'no keep current'                                                              => 'خیر، فعلی را نگه دارم',
    'error saving translation preference'                                           => 'خطا در ذخیره تنظیمات ترجمه',
    'incomplete translations marked with warning'                                  => 'ترجمه‌های ناقص با علامت ⚠️ مشخص شده‌اند',
    'settings menu'                                                                 => 'منوی تنظیمات',
    'select translation language'                                                  => 'انتخاب زبان ترجمه',
    'current translation language'                                                 => 'زبان ترجمه فعلی',
    'select option from menu'                                                      => 'گزینه مورد نظر را از منو انتخاب کنید',
    'view translations for current language'                                         => 'مشاهده ترجمه‌های زبان فعلی',
    'help'                                                                        => [
        'choose :reciter as reciter'                           => ':reciter را به عنوان قاری انتخاب کنید',
        'choose :language as transliteration'                  => ':language را به عنوان نویسه گردان انتخاب کنید',
        'choose :translator as translation'                    => ':translator را به عنوان ترجمه انتخاب کنید',
        'list of this robot commands'                          => 'لیستی از دستورات این ربات',
        'your quran readings analysis report'                  => 'گزارش تحلیل قرائت قرآن شما',
        'dont show :language transliteration'                  => 'نویسه نویسی :language را نشان ندهید',
        'disable sending mp3 for every ayah'                   => 'غیرفعال کردن ارسال mp3 برای هر آیه',
        'send mp3 for selected reciter'                        => 'ارسال mp3 برای قاری منتخب',
        'view available translations'                          => 'مشاهده ترجمه‌های موجود',
        'settings menu'                                         => 'منوی تنظیمات',
        'list of Surahs of the Quran'                          => 'فهرست سوره های قرآن',
        'list of Quran 30 parts'                               => 'فهرست قرآن 30 جزء',
        'to help you for introduce all features of this robot' => 'تا به شما در معرفی تمامی ویژگی های این ربات کمک کند',
        'to send scanned quran page'                           => 'برای ارسال صفحه قرآن اسکن شده',
        'help'                                                 => 'کمک',
    ],
    'translators'                                                                 => [
        'ansarian' => 'انصاریان',
        'ayati'    => 'آیتی',
    ],
    'transliterations'                                                            => [
        'english' => 'انگلیسی',
        'turkish' => 'ترکی',
    ],
    'reciters'                                                                    => [
        'parhizgar' => 'پرهیزگار',
        'alafasy'   => 'العفاسی',
    ],
    'bots'                                                                        => [
        'getchatidbot' => [
            'our other bots' => 'ربات های دیگر ما',
        ],
    ],
    'to send your daily activity report please try it with this command'          => 'برای ارسال گزارش فعالیت روزانه خود لطفا با این دستور آن را امتحان کنید',
    'referral code'                                                               => 'کد ارجاع',
    'here is your referral link'                                                  => 'اینجا لینک ارجاع شماست',
    'report'                                                                      => [
        'this is your reports' => [
            ' your last 7 days activities' => [
                ' click on this link:' => 'این گزارش های شماست فعالیت های 7 روز گذشته شما روی این لینک کلیک کنید:',
            ],
        ],
        'last 7 days readings' => 'آمار مطالعه شما در 7 روز گذشته',
    ],
    'next quran page click here'                                                  => 'برای صفحه بعدی قرآن اینجا کلیک کنید',
    'sent it for :count person'                                                   => 'آن را برای :count از افراد ارسال کردم.',
    'for next or previous quran page click on these buttons'                      => 'برای صفحه قرآن بعدی یا قبلی روی این دکمه ها کلیک کنید',
    'previous quran page click here'                                              => 'برای دریافت صفحه قبل قرآن اینجا را کلیک کنید',
    'page'                                                                        => 'صفحه',
    'to sending request for next result page please click here'                   => 'برای ارسال درخواست برای صفحه نتیجه بعدی لطفا اینجا را کلیک کنید',
    'not found'                                                                   => 'پیدا نشد',
    'command is too long for process'                                             => 'اجرای این دستور برای پردازش خیلی طولانی شد',
    'if there is no results please try again with non long query with less words' => [
        ' thank you' => 'اگر نتیجه ای حاصل نشد، لطفاً با پرس و جوی غیر طولانی با کلمات کمتر دوباره امتحان کنید. متشکرم',
    ],
    'your last activities' => 'آخرین فعالیت‌های شما',
    'hadith of the day' => 'حدیث روز',
    'the last verse you were reading' => 'آخرین آیه‌ای که می‌خوندی',
    'continue' => 'ادامه بده',
    'you have completed the quran' => 'شما قرآن را به پایان رساندید!',
    'this command not work in telegram' => 'این دستور در تلگرام کار نمی‌کند',
    'you are not admin' => 'شما ادمین نیستید',
    'statistics report' => 'گزارش آمار ربات قرآن',
    'daily statistics' => 'آمار روزانه',
    'weekly statistics' => 'آمار هفتگی',
    'monthly statistics' => 'آمار ماهانه',
    'yearly statistics' => 'آمار سالانه',
    'total ayah' => 'تعداد کل آیه',
    'unique users' => 'کاربران منحصر به فرد',
    'you had no reading today and yesterday' => 'شما امروز و دیروز هیچ مطالعه‌ای نداشتید',
    'suggested verse from other users today' => 'آیه پیشنهادی از فعالیت‌های امروز دیگر کاربران',
    'start from here' => 'از اینجا شروع کنید',
    'last activities' => 'آخرین فعالیت‌ها',
    'return to menu' => 'بازگشت به فهرست',
    'yesterday with users you read rounds' => 'در روز گذشته به همراه :users_count نفر، مجموعاً حدود :rounds دور کامل قرآن رو قرائت کرده‌اید.',
    'referral statistics message' => 'طی ۷ روز گذشته شما و :invitees_count فرد دعوت‌شده توسط شما، مجموعاً :total_ayahs آیه رو قرائت کرده‌اید.',
    'how to invite others' => 'چطوری از دیگران دعوت کنم؟',
    'just forward this message' => 'کافیه همین پیام رو فوروارد کنین!',
    'invitation message' => 'سلام' . "\n" . 'أَلَا بِذِكْرِ اللَّهِ تَطْمَئِنُّ الْقُلُوبُ (آیه ۲۸ سوره‌ی رعد)' . "\n" . 'من و چند هزار کاربر از بازوی دستیار قرآنی استفاده می‌کنیم و علاوه بر امکاناتی مثل کمک به مرور منظم قرآن، جستجو، دسترسی به تفسیر، شان نزول، استخاره و ... هر روز در کنار هم حدوداً ۷ بار قرآن را قرائت می‌کنیم.' . "\n" . 'پیشنهاد می‌کنم شما هم به دعوتم لبیک بگی😊',
    'accept invitation' => 'لبیک به دعوت',
    'copy invitation link' => 'رونوشت لینک اختصاصی',
    'complete quran rounds' => 'دور کامل قرآن',
    'invited users' => 'کاربران دعوت‌شده',
    'invited users count' => 'تعداد کاربران دعوت‌شده',
    'invited users active last 7 days' => 'کاربران دعوت‌شده فعال در 7 روز گذشته',
    'your invitation link' => 'لینک دعوت شما',
    'error generating invitation link' => 'خطا در ساخت لینک دعوت',
    'presenter' => [
        'next' => 'بعدی',
        'end' => 'پایان معرفی',
        'send_content' => 'لطفاً محتوای ربات را ارسال کنید (هر خط یک پیام)',
    ],
    'rating' => [
        'prompt' => 'چقدر این عبارت را قبول داری؟',
        'end' => 'پایان نظر سنجی',
        'send_content' => 'لطفاً عبارت‌های نظر سنجی را ارسال کنید (هر خط یک عبارت)',
    ],
    'your activity report' => 'گزارش فعالیت شما',
    'your reading is more than yesterday activity' => ':count آیه مطالعه شما بیشتر از فعالیت دیروز است',
    'your reading is less than yesterday activity' => ':count آیه مطالعه شما کمتر از فعالیت دیروز است',
    'your reading is equal to yesterday activity' => 'تعداد آیه‌های مطالعه شما برابر با فعالیت دیروز است',
    'environment' => 'محیط: :env',
    'prayer blessing' => 'اللهم صل علی محمد و آل محمد و عجل فرجهم',
    'prayer forgiveness' => 'استغفر الله ربی و اتوب الیه',
    'quran_bots' => [
        'intro' => [
            'title' => '✨ مژده به پویندگان راه قرآن ✨',
            'body' => 'با عنایت الهی و تکیه بر توان متخصصان، ربات هوشمند «حفظ و ختم قرآن کریم» در تلگرام، اکنون با پشتیبانی از :count زبان زنده دنیا آماده خدمت‌رسانی به مسلمانان و حق‌جویان سراسر جهان است.',
            'bots_intro' => 'شما می‌توانید بر اساس زبان مورد نظر خود، از طریق لینک‌های زیر وارد ربات شوید:',
            'bots_title' => '🔹 نسخه‌های زبانی ربات:',
            'channels_intro' => '📢 آخرین اخبار و ویژگی‌های جدید: برای اطلاع از بروزرسانی‌ها و امکانات تازه‌ی ربات، در کانال‌های اطلاع‌رسانی ما عضو شوید:',
        ],
        'select_language' => 'برای چه زبانی می‌خواهید متن معرفی ربات‌های قرآن را ببینید؟',
        'select_source' => 'از چه منبعی استفاده کنیم؟',
        'source' => [
            'both' => 'ترکیب کانفیگ + دیتابیس',
            'database_only' => 'فقط دیتابیس',
        ],
        'language_name' => [
            'ar-IQ' => 'عربی',
            'ur' => 'اردو',
            'zh-CN' => 'چینی',
            'es' => 'اسپانیایی',
            'de-DE' => 'آلمانی',
            'fr' => 'فرانسوی',
            'en' => 'انگلیسی',
            'fa' => 'فارسی',
            'ru' => 'روسی',
            'tr' => 'ترکی',
            'he' => 'عبری',
        ],
    ],
    
    // ترجمه‌های ربات نماز قضا
    'prayer_recorded' => 'رکعت ثبت شد',
    'prayer_removed' => 'رکعت حذف شد',
    'prayer_not_found' => 'رکعت یافت نشد',
    'prayer_type' => 'نوع نماز',
    'record_id' => 'شناسه',
    'to_remove' => 'برای حذف',
    'check_record_id' => 'شناسه را بررسی کنید',
    'error_recording_prayer' => 'خطا در ثبت رکعت',
    'error_removing_prayer' => 'خطا در حذف رکعت',
    'error_getting_stats' => 'خطا در دریافت آمار',
    'error_setting_estimate' => 'خطا در ثبت تخمین',
    
    // نماز‌ها
    'fajr' => 'صبح',
    'dhuhr' => 'ظهر',
    'asr' => 'عصر',
    'maghrib' => 'مغرب',
    'isha' => 'عشا',
    'maghrib_isha' => 'مغرب و عشا',
    'dhuhr_asr' => 'ظهر و عصر',

    // آمار
    'weekly_stats' => 'آمار هفتگی',
    'total_rakats' => 'مجموع رکعات',
    'total_records' => 'تعداد ثبت',
    'last_week' => 'هفته گذشته',
    'rakats' => 'رکعت',
    'records' => 'ثبت',
    'by_prayer_type' => 'به تفکیک نماز',
    
    // پیشرفت
    'progress' => 'پیشرفت',
    'goal' => 'هدف',
    'completed' => 'انجام شده',
    'remaining' => 'باقی‌مانده',
    'percentage' => 'درصد',
    'almost_done' => 'تقریباً تمام شد! عالی پیش می‌روید!',
    'great_progress' => 'پیشرفت عالی! ادامه دهید!',
    'keep_going' => 'همین‌طور ادامه دهید!',
    'good_start' => 'شروع خوبی بوده! ادامه دهید!',
    
    // تخمین
    'estimate_set' => 'تخمین ثبت شد',
    'total_prayers' => 'تعداد نماز',
    'start_recording_now' => 'حالا شروع به ثبت کنید!',
    'estimate_usage' => 'نحوه استفاده',
    'example' => 'مثال',
    'or' => 'یا',
    'estimate_cancelled' => '❌ عملیات لغو شد.',
    'estimate_unit_selected' => '📊 تخمین بر اساس :unit\n\nچند :unit نماز قضا دارید؟\nلطفاً یک عدد ارسال کنید:\n\nمثال: 6',
    'estimate_invalid_number' => '❌ لطفاً یک عدد معتبر (بزرگتر از صفر) ارسال کنید.',
    'estimate_saved' => '✅ تخمین شما ثبت شد!\n\n📊 تخمین جدید:\n• :value :unit\n• حدود :rakats رکعت\n\n💡 این تخمین به عنوان هدف شما ذخیره شد و در گزارش‌های هفتگی نمایش داده می‌شود.\n\nبرای مشاهده آمار: /stats',
    
    // واحدها
    'unit_day' => 'روز',
    'unit_week' => 'هفته',
    'unit_month' => 'ماه',
    'unit_year' => 'سال',
    'unit_rakat' => 'رکعت',
    
    // عمومی
    'cancel' => 'انصراف',
    
    // پیام‌های ربات
    'welcome_prayer_bot' => 'به ربات نماز قضا خوش آمدید',
    'prayer_bot_description' => 'با این ربات می‌توانید نمازهای قضای خود را ثبت کنید',
    'send_number_instructions' => 'فقط عدد بفرستید',
    'available_commands' => 'دستورات موجود',
    'view_stats' => 'مشاهده آمار',
    'set_estimate' => 'ثبت تخمین',
    'email_settings' => 'تنظیمات ایمیل',
    'help' => 'راهنما',
    
    // راهنما
    'prayer_bot_help' => 'راهنمای ربات نماز قضا',
    'how_to_record' => 'نحوه ثبت',
    'just_send_number' => 'فقط عدد 2، 3 یا 4 بفرستید',
    'how_to_remove' => 'نحوه حذف',
    'use_remove_command' => 'از دستور /remove_<id> استفاده کنید',
    'how_to_estimate' => 'نحوه ثبت تخمین',
    'how_to_email' => 'نحوه تنظیم ایمیل',
    'smart_detection' => 'تشخیص هوشمند',
    'smart_detection_description' => 'ربات به صورت هوشمند نوع نماز را تشخیص می‌دهد',
    
    // پیام‌های تشویقی
    'encouragement_1' => '🌟 ماشاءالله! ادامه دهید!',
    'encouragement_2' => '💪 عالی است! به همین منوال پیش بروید!',
    'encouragement_3' => '✨ خداوند قبول فرماید!',
    'encouragement_4' => '🎯 به هدفتان نزدیک می‌شوید!',
    'encouragement_5' => '🙏 خداوند یاور و مددکار شما باشد!',
    'encouragement_6' => '💚 هر قدمی ارزشمند است!',
    'encouragement_7' => '🌙 خداوند توفیق روزافزون عطا فرماید!',
    
    // ایمیل
    'email_settings_title' => 'تنظیمات ایمیل',
    'email_settings_description' => 'ایمیل خود را ثبت کنید تا گزارش هفتگی دریافت کنید',
    'email_already_set' => 'ایمیل شما تنظیم شده است:',
    'email_change_instructions' => 'اگر می‌خواهید تغییر دهید، از دکمه‌های زیر استفاده کنید:',
    'send_your_email' => 'ایمیل خود را بفرستید',
    'email_settings_coming_soon' => 'به زودی...',
    'daily' => 'روزانه',
    'weekly' => 'هفتگی',
    'monthly' => 'ماهانه',
    'never' => 'هرگز',
    'set_email' => 'ثبت ایمیل',
    
    // پیام‌های خطا
    'prayer_bot_unknown_command' => 'دستور نامشخص',
    'send_number_to_record' => 'برای ثبت، عدد بفرستید',
    'or_use_commands' => 'یا از دستورات استفاده کنید',
    
    // ==================== سیستم Help ====================
    
    // Help اصلی
    'help_main_title' => '🕌 راهنمای ربات نماز قضا',
    'help_main_welcome' => 'به ربات نماز قضا خوش آمدید! این ربات به شما کمک می‌کند تا:',
    'help_main_features' => "✅ نمازهای قضای خود را ثبت کنید\n📊 تخمین نمازهای قضا را مدیریت کنید\n📧 گزارش هفتگی دریافت کنید\n📈 پیشرفت خود را پیگیری کنید",
    'help_main_select' => '📚 برای اطلاعات بیشتر، یکی از موارد زیر را انتخاب کنید:',
    'help_main_quick_start' => '💡 برای شروع سریع: فقط عدد 2، 3 یا 4 ارسال کنید!',
    
    // دکمه‌های Help
    'help_btn_commands' => '📋 دستورات',
    'help_btn_usage' => '📖 نحوه استفاده',
    'help_btn_estimate' => '📊 تخمین',
    'help_btn_report' => '📧 گزارش ایمیل',
    'help_btn_faq' => '❓ سوالات متداول',
    'help_btn_back_to_help' => '◀️ بازگشت به راهنما',
    'help_btn_back_to_menu' => '🏠 منوی اصلی',
    
    // Help دستورات
    'help_commands_title' => '📋 لیست دستورات',
    'help_commands_main' => "🔹 دستورات اصلی:\n• /start - شروع کار با ربات\n• /help - نمایش راهنما\n• /stats - مشاهده آمار",
    'help_commands_record' => "🔹 ثبت نماز:\n• ارسال عدد 2 - ثبت نماز صبح (2 رکعت)\n• ارسال عدد 3 - ثبت نماز مغرب (3 رکعت)\n• ارسال عدد 4 - ثبت نماز ظهر/عصر/عشا (4 رکعت)\n• /remove_[ID] - حذف نماز ثبت شده",
    'help_commands_estimate' => "🔹 تخمین:\n• /estimate - ثبت تخمین نمازهای قضا\n• /estimate_status - مشاهده تخمین فعلی",
    'help_commands_report' => "🔹 گزارش:\n• /set_email - تنظیم ایمیل برای گزارش\n• /email_settings - تنظیمات ایمیل\n• /unsubscribe - لغو اشتراک ایمیل",
    'help_commands_help' => "🔹 راهنما:\n• /help_usage - نحوه استفاده\n• /help_estimate - راهنمای تخمین\n• /help_report - راهنمای گزارش\n• /help_faq - سوالات متداول",
    'help_commands_tip' => '💡 نکته: می‌توانید به جای ارسال دستور، از دکمه‌های زیر پیام استفاده کنید.',
    
    // Help نحوه استفاده
    'help_usage_title' => '📖 نحوه استفاده از ربات',
    'help_usage_content' => "🔸 مرحله 1: ثبت نماز\nبرای ثبت نماز قضا، کافی است تعداد رکعات را ارسال کنید:\n\n• 2 → نماز صبح (2 رکعت)\n• 3 → نماز مغرب (3 رکعت)\n• 4 → نماز ظهر، عصر یا عشا (4 رکعت)\n\nمثال:\nشما: 4\nربات: ✅ 4 رکعت (نماز ظهر) ثبت شد!\n\n🔸 مرحله 2: مشاهده آمار\nبا دستور /stats می‌توانید آمار خود را ببینید:\n• تعداد کل رکعات ثبت شده\n• تعداد نمازهای ثبت شده\n• پیشرفت نسبت به تخمین\n• آمار هفته جاری\n\n🔸 مرحله 3: ثبت تخمین (اختیاری)\nبرای پیگیری بهتر، تخمین نمازهای قضای خود را ثبت کنید:\n1. دستور /estimate را ارسال کنید\n2. واحد (روز، هفته، ماه، سال، رکعت) را انتخاب کنید\n3. عدد را وارد کنید\n\n🔸 مرحله 4: دریافت گزارش (اختیاری)\nبرای دریافت گزارش هفتگی:\n1. دستور /set_email را ارسال کنید\n2. ایمیل خود را وارد کنید\n3. کد تایید را وارد کنید\n\n💡 نکته: ربات به صورت هوشمند نوع نماز را تشخیص می‌دهد.",
    
    // Help تخمین
    'help_estimate_title' => '📊 راهنمای سیستم تخمین',
    'help_estimate_what' => '🎯 تخمین چیست؟\nتخمین، مجموع نمازهای قضایی است که فکر می‌کنید دارید.\nاین عدد به شما کمک می‌کند پیشرفت خود را پیگیری کنید.',
    'help_estimate_how' => "📝 نحوه ثبت تخمین:\n\n1️⃣ دستور /estimate را ارسال کنید\n\n2️⃣ واحد مورد نظر را انتخاب کنید:\n   • روز: تعداد روزهایی که نماز نخوانده‌اید\n   • هفته: تعداد هفته‌ها\n   • ماه: تعداد ماه‌ها\n   • سال: تعداد سال‌ها\n   • رکعت: مستقیم تعداد رکعات\n\n3️⃣ عدد را وارد کنید\n\nمثال:\nشما: /estimate\nربات: [نمایش دکمه‌ها]\nشما: [کلیک روی \"ماه\"]\nربات: چند ماه؟\nشما: 6\nربات: ✅ 6 ماه = 3060 رکعت ثبت شد",
    'help_estimate_calc' => "🔢 محاسبات:\n• هر روز = 17 رکعت (5 نماز)\n• هر هفته = 119 رکعت (7 روز)\n• هر ماه = 510 رکعت (30 روز)\n• هر سال = 6205 رکعت (365 روز)",
    'help_estimate_progress' => "📈 نمایش پیشرفت:\nبعد از ثبت تخمین، در دستور /stats می‌بینید:\n\n🎯 تخمین کل: 3060 رکعت\n✅ ثبت شده: 450 رکعت (15%)\n📉 باقیمانده: 2610 رکعت\n\n⏱️ زمان تخمینی: با این سرعت، حدود 8 ماه",
    'help_estimate_tip' => '💡 نکته: تخمین جدید جایگزین تخمین قبلی می‌شود.',
    
    // Help گزارش
    'help_report_title' => '📧 راهنمای سیستم گزارش ایمیل',
    'help_report_what' => "📬 گزارش هفتگی چیست؟\nهر هفته یک ایمیل حاوی:\n• تعداد نمازهای ثبت شده در هفته\n• پیشرفت نسبت به تخمین\n• نمودار پیشرفت\n• انگیزه و تشویق",
    'help_report_how' => "⚙️ نحوه فعال‌سازی:\n\n1️⃣ تنظیم ایمیل:\n   /set_email\n\n2️⃣ وارد کردن ایمیل:\n   example@gmail.com\n\n3️⃣ تایید ایمیل:\n   کد 6 رقمی ارسال شده را وارد کنید\n\n✅ فعال شد! از این پس هر هفته گزارش دریافت می‌کنید.",
    'help_report_settings' => "🔧 تنظیمات:\n\n• مشاهده تنظیمات: /email_settings\n• تغییر ایمیل: /set_email (ایمیل جدید)\n• لغو اشتراک: /unsubscribe",
    'help_report_time' => '📅 زمان ارسال:\nگزارش‌ها هر یکشنبه صبح ساعت 9 ارسال می‌شوند.',
    'help_report_security' => "🔒 امنیت:\n• ایمیل شما محفوظ است\n• فقط برای ارسال گزارش استفاده می‌شود\n• هر زمان می‌توانید لغو کنید",
    'help_report_tip' => '💡 نکته: حتی اگر هفته‌ای نماز ثبت نکنید، ایمیل انگیزشی دریافت می‌کنید!',
    'help_btn_get_report_link' => '🔗 دریافت لینک گزارش',
    'help_report_link_available' => '✅ لینک گزارش شما:',
    'help_report_need_estimate' => '⚠️ برای دریافت لینک گزارش، ابتدا باید تخمین نماز قضای خود را ثبت کنید.\n\nاز دستور /estimate استفاده کنید.',
    'help_report_need_estimate_message' => '⚠️ برای دریافت لینک گزارش، ابتدا باید تخمین نماز قضای خود را ثبت کنید.\n\nاز دستور /estimate استفاده کنید.',
    'help_report_link_sent' => '✅ لینک گزارش شما:',
    'help_report_link_instruction' => '💡 می‌توانید این لینک را در مرورگر باز کنید و گزارش کامل با نمودارها را مشاهده کنید.',
    
    // Help سوالات متداول
    'help_faq_title' => '❓ سوالات متداول (FAQ)',
    'help_faq_content' => "🔹 چگونه نماز ثبت کنم؟\nفقط عدد 2، 3 یا 4 را ارسال کنید.\n2 = صبح، 3 = مغرب، 4 = ظهر/عصر/عشا\n\n🔹 اگر اشتباه ثبت کردم چه کنم؟\nاز دستور /remove_[ID] استفاده کنید.\nID در پیام تایید ربات نمایش داده می‌شود.\n\n🔹 ربات چطور نوع نماز را تشخیص می‌دهد؟\nبر اساس تعداد رکعات و ساعت روز:\n• 2 رکعت → صبح\n• 3 رکعت → مغرب\n• 4 رکعت → ظهر (قبل از ظهر) یا عصر/عشا (بعد از ظهر)\n\n🔹 آیا باید تخمین ثبت کنم؟\nخیر، اختیاری است. ولی برای پیگیری بهتر توصیه می‌شود.\n\n🔹 تخمین من دقیق نیست، چه کنم؟\nمی‌توانید هر زمان با /estimate تخمین جدید ثبت کنید.\n\n🔹 چند بار در روز می‌توانم نماز ثبت کنم؟\nنامحدود! هر چند نماز که می‌خوانید ثبت کنید.\n\n🔹 آیا می‌توانم نمازهای قبلی را ببینم؟\nبله، با دستور /stats لیست آخرین نمازها را می‌بینید.\n\n🔹 گزارش ایمیل چه زمانی ارسال می‌شود؟\nهر یکشنبه صبح ساعت 9\n\n🔹 اگر ایمیل دریافت نکردم؟\n• پوشه Spam را چک کنید\n• ایمیل را با /email_settings بررسی کنید\n• ایمیل جدید با /set_email ثبت کنید\n\n🔹 آیا اطلاعات من محفوظ است؟\nبله، تمام اطلاعات شما محرمانه و امن است.\n\n🔹 ربات رایگان است؟\nبله، کاملاً رایگان و بدون محدودیت!\n\n🔹 آیا ربات در بله هم کار می‌کند؟\nبله، در تلگرام و بله هر دو کار می‌کند.\n\n💡 سوال دیگری دارید؟\nبا پشتیبانی تماس بگیرید: @support",
    
    // ==================== تخمین Status ====================
    'estimate_status_title' => 'وضعیت تخمین',
    'estimate_status_no_estimate' => 'هنوز تخمینی ثبت نکرده‌اید.\n\nبرای ثبت تخمین از دستور /estimate استفاده کنید.',
    'estimate_status_total' => 'تخمین کل',
    'estimate_status_equivalent' => 'معادل',
    'estimate_status_recorded' => 'ثبت شده',
    'estimate_status_remaining' => 'باقیمانده',
    'estimate_status_progress' => 'پیشرفت',
    
    // ==================== ایمیل ====================
    'email_format_example' => 'مثال: example@gmail.com',
    'email_invalid_format' => '❌ فرمت ایمیل نامعتبر است.\n\nلطفاً یک ایمیل معتبر ارسال کنید.\nمثال: example@gmail.com',
    'email_code_sent' => 'کد تایید به ایمیل شما ارسال شد',
    'email_enter_code' => 'لطفاً کد 6 رقمی ارسال شده را وارد کنید:',
    'email_code_expires' => 'این کد تا',
    'minutes' => 'دقیقه',
    'email_code_invalid_format' => '❌ کد باید 6 رقم باشد.\n\nلطفاً کد 6 رقمی را ارسال کنید.',
    'email_code_incorrect' => '❌ کد تایید اشتباه است.\n\nلطفاً دوباره تلاش کنید.',
    'email_code_expired' => '❌ کد تایید منقضی شده است.\n\nلطفاً از دستور /set_email استفاده کنید و دوباره تلاش کنید.',
    'email_verified_success' => 'ایمیل شما با موفقیت تایید شد!',
    'email_verified_message' => 'از این پس گزارش‌های هفتگی به این ایمیل ارسال می‌شود.',
    'email_report_frequency' => 'فرکانس گزارش',
    'email_send_error' => '❌ خطا در ارسال ایمیل.\n\nلطفاً دوباره تلاش کنید یا با پشتیبانی تماس بگیرید.',
    'email_verification_error' => 'خطا در تایید ایمیل',
    'please_try_again' => 'لطفاً دوباره تلاش کنید یا با پشتیبانی تماس بگیرید.',
    
    // تنظیمات ایمیل
    'email_settings_current_email' => 'ایمیل فعلی',
    'email_settings_verified' => 'تایید شده',
    'email_settings_not_verified' => 'تایید نشده',
    'email_settings_no_email' => 'ایمیل ثبت نشده',
    'email_settings_frequency' => 'فرکانس گزارش',
    'email_settings_instructions' => 'برای تغییر فرکانس گزارش، یکی از گزینه‌های زیر را انتخاب کنید:',
    'email_settings_unsubscribe' => 'لغو اشتراک',
    'email_settings_change_email' => 'تغییر ایمیل',
    'email_frequency_updated' => 'فرکانس گزارش به‌روزرسانی شد',
    'email_unsubscribed' => 'اشتراک شما لغو شد',
    'email_unsubscribed_message' => 'دیگر گزارش‌های ایمیل دریافت نخواهید کرد.\n\nبرای فعال‌سازی مجدد از دستور /set_email استفاده کنید.',
    
    // ایمیل تایید (برای view)
    'email_verification_subject' => 'کد تایید ایمیل - ربات نماز قضا',
    'email_verification_title' => 'کد تایید ایمیل',
    'email_verification_greeting' => 'سلام،',
    'email_verification_message' => 'کد تایید ایمیل شما برای ربات نماز قضا:',
    'email_verification_warning' => '⚠️ این کد تا 10 دقیقه معتبر است.\n⚠️ اگر شما این درخواست را نکرده‌اید، این ایمیل را نادیده بگیرید.',
    'email_verification_footer' => 'این ایمیل به صورت خودکار ارسال شده است. لطفاً به آن پاسخ ندهید.',
    
    // ==================== Location ====================
    'location_request' => '📍 لطفاً موقعیت مکانی خود را ارسال کنید.\n\nبرای ارسال location:\n1. روی آیکون 📎 کلیک کنید\n2. Location را انتخاب کنید\n3. موقعیت خود را ارسال کنید',
    'location_saved' => '✅ موقعیت مکانی شما با موفقیت ذخیره شد',
    'location_not_set' => '❌ موقعیت مکانی تنظیم نشده است.\n\nلطفاً از دستور /location استفاده کنید و موقعیت خود را ارسال کنید.',
    'location_current' => 'موقعیت فعلی',
    'location_address' => 'آدرس',
    'location_invalid' => '❌ موقعیت ارسال شده نامعتبر است.\n\nلطفاً دوباره تلاش کنید.',
    
    // ==================== Weather Alerts ====================
    'alert_list' => 'لیست هشدارها',
    'no_alerts' => 'هیچ هشداری ثبت نشده است.',
    'alert_add' => 'افزودن هشدار',
    'alert_deleted' => '✅ هشدار حذف شد',
    'alert_not_found' => '❌ هشدار یافت نشد',
    'alert_created' => '✅ هشدار با موفقیت ایجاد شد',
    'alert_limit_reached' => '❌ شما به حداکثر تعداد هشدار ({max} عدد) رسیده‌اید.\n\nبرای افزودن هشدارهای بیشتر، از امکانات Pro استفاده کنید.',
    'alert_triggered' => 'هشدار فعال شد',
    'alert_location' => 'موقعیت',
    'alert_count' => 'تعداد هشدارها: {current} / {max}',
    'alert_select_type' => 'نوع هشدار را انتخاب کنید:',
    'alert_temperature' => 'دما',
    'alert_precipitation' => 'بارش',
    'alert_wind' => 'باد',
    'alert_snow' => 'برف',
    'alert_type_temperature' => 'دما',
    'alert_type_precipitation' => 'بارش',
    'alert_type_wind' => 'باد',
    'alert_type_snow' => 'برف',
    'alert_comparison_increase' => 'افزایش',
    'alert_comparison_decrease' => 'کاهش',
    'alert_comparison_absolute' => 'مطلق',
    'alert_set_threshold_temperature' => 'لطفاً حد تغییرات دما را تعیین کنید (مثلاً 5 برای 5 درجه):',
    'alert_set_threshold_precipitation' => 'لطفاً حد تغییرات بارش را تعیین کنید (مثلاً 10 برای 10 میلی‌متر):',
    'alert_set_threshold_wind' => 'لطفاً حد تغییرات باد را تعیین کنید (مثلاً 10 برای 10 کیلومتر بر ساعت):',
    'alert_set_threshold_snow' => 'لطفاً حد تغییرات برف را تعیین کنید (مثلاً 5 برای 5 میلی‌متر):',
    'alert_example_threshold' => 'مثال: 5 (برای تغییرات 5 درجه یا 5 میلی‌متر)',
    'unlimited' => 'نامحدود',
    'delete' => 'حذف',
    
    // ==================== Pro ====================
    'pro_status' => 'وضعیت Pro',
    'pro_active' => 'Pro فعال است',
    'pro_not_active' => 'Pro فعال نیست',
    'pro_expires_at' => 'تاریخ انقضا',
    'pro_buy' => 'خرید Pro',
    'pro_features_description' => "با Pro می‌توانید:" . PHP_EOL . "• هشدارهای نامحدود" . PHP_EOL . "• گزارش‌های پیشرفته" . PHP_EOL . "• و امکانات بیشتر",
    'pro_required' => 'این قابلیت نیاز به Pro دارد',
    'pro_feature_unlimited_alerts' => 'هشدارهای نامحدود',
    'pro_buy_instruction' => 'برای خرید Pro از دستور /pro_buy استفاده کنید',
    'pro_purchase_requested' => '✅ درخواست خرید Pro شما ثبت شد',
    'pro_contact_admin' => 'لطفاً با ادمین تماس بگیرید: :admin',
    'pro_payment_info' => 'پس از واریز وجه، اطلاعات پرداخت را برای ادمین ارسال کنید.',
    'pro_purchase_error' => '❌ خطا در ثبت درخواست خرید',
    'command_not_found' => '❌ دستور یافت نشد\n\n',
    'weather_error' => '❌ خطا در دریافت اطلاعات هواشناسی',
    'no_weather_data' => 'هیچ داده هواشناسی دریافت نشد',

    // Poem Bot translations
    'poem_bot_welcome' => '📝 خوش آمدید به ربات شعر و موسیقی!\n\nبا این ربات می‌توانید اشعار خود را ارسال، ویرایش و به اشتراک بگذارید.',
    'poem_bot_new_poem' => '📝 ارسال شعر جدید',
    'poem_bot_edit_poem' => '✏️ ویرایش شعر',
    'poem_bot_my_poems' => '📚 شعرهای من',
    'poem_bot_likes' => '❤️ لایک‌ها',
    'poem_bot_list' => '🔍 مشاهده فهرست',
    'poem_bot_suggest' => '💡 پیشنهاد مصرع',
    'poem_bot_help' => 'ℹ️ راهنما',
    'poem_bot_select_type' => 'لطفاً نوع شعر را انتخاب کنید:',
    'poem_bot_classic' => 'شعر کلاسیک',
    'poem_bot_novel' => 'شعر نو',
    'poem_bot_line' => 'مصرع',
    'poem_bot_sentence' => 'جمله',
    'poem_bot_send_line' => 'لطفاً {line_type} خود را ارسال کنید:',
    'poem_bot_line_added' => '✅ {line_type} اضافه شد. تعداد: {count}',
    'poem_bot_add_more' => '➕ افزودن {line_type} جدید',
    'poem_bot_finish' => '✅ پایان و انتشار',
    'poem_bot_no_poems' => 'شما هنوز شعری ارسال نکرده‌اید.',
    'poem_bot_select_poem_to_edit' => 'لطفاً شعری را برای ویرایش انتخاب کنید:',
    'poem_bot_untitled' => 'بدون عنوان',
    'poem_bot_liked' => '✅ شعر لایک شد!',
    'poem_bot_already_liked' => 'شما قبلاً این شعر را لایک کرده‌اید.',
    'poem_bot_error' => '❌ خطا در انجام عملیات. لطفاً دوباره تلاش کنید.',
    'poem_bot_no_lines' => 'شما هیچ {line_type} اضافه نکرده‌اید.',
    'poem_bot_poem_created' => '✅ شعر با موفقیت ایجاد شد!\n\nشناسه شعر: {id}',
    'poem_bot_publish' => '🚀 انتشار',
    'poem_bot_save_draft' => '💾 ذخیره به عنوان پیش‌نویس',
    'poem_bot_editing_not_implemented' => 'ویژگی ویرایش در حال توسعه است.',
    'poem_bot_my_poems_list' => '📚 شعرهای شما ({count}):',
    'poem_bot_likes_list' => '❤️ شعرهای لایک شده شما:',
    'poem_bot_no_published_poems' => 'هنوز هیچ شعری منتشر نشده است.',
    'poem_bot_published_poems' => '📚 اشعار منتشر شده:',
    'poem_bot_sort_by_likes' => '📊 بر اساس لایک',
    'poem_bot_sort_by_date' => '📅 بر اساس تاریخ',
    'poem_bot_suggest_line_instruction' => 'لطفاً شناسه شعر و مصرع پیشنهادی خود را ارسال کنید.',
    'poem_bot_help_text' => '📖 راهنمای ربات شعر:\n\n/start - شروع\n/newpoem - ارسال شعر جدید\n/editpoem - ویرایش شعر\n/like [id] - لایک کردن شعر\n/help - راهنما',
    'poem_bot_about' => '📝 ربات شعر و موسیقی\n\nاین ربات به شما امکان ارسال، ویرایش و به اشتراک‌گذاری اشعار را می‌دهد.',
    'poem_bot_profile' => '👤 پروفایل شما:\n\n📚 تعداد اشعار: {poems_count}',

    'admin_kie_submitted' => '✅ درخواست شما ثبت شد و در حال بررسی است.',
    'admin_kie_pending' => "⏳ درخواست قبلی شما (ID: :id) هنوز در حال بررسی است.\n\nاگر ادمین هستید:\n/adminbot_confirm :id\n/adminbot_reject :id",
    'admin_kie_already_owner' => 'ℹ️ شما قبلاً ادمین این ربات هستید.',
    'admin_kie_approved_user' => '✅ دسترسی ادمین ربات برای شما فعال شد.',
    'admin_kie_confirmed_admin' => '✅ درخواست adminkie تایید شد. کاربر اکنون ادمین ربات است.',
    'admin_kie_rejected_admin' => '❌ درخواست adminkie رد شد.',
    'admin_kie_confirm_usage' => "❌ فرمت دستور اشتباه است.\n\nاستفاده:\n/adminbot_confirm [REQUEST_ID]\n/adminbot_confirm [REQUEST_ID] [MAIN_BOT_ID]",
    'admin_kie_reject_usage' => "❌ فرمت دستور اشتباه است.\n\nاستفاده: /adminbot_reject [REQUEST_ID]",
    'admin_kie_bot_id_missing' => 'ربات مشخص نیست؛ bot_id در درخواست خالی است.',
    'admin_kie_bot_not_registered' => "❌ ربات در سیستم ثبت نشده یا شناسه webhook منقضی شده.\n\nلطفاً از ربات اصلی کتابخانه (نه reader) /adminkie بزنید یا با پشتیبانی تماس بگیرید.",
    'admin_kie_bot_not_found' => "❌ ربات با شناسه :id یافت نشد (Request ID: :request_id).\n\nتایید دستی:\n/adminbot_confirm :request_id [MAIN_BOT_ID]\n\nیا رد:\n/adminbot_reject :request_id",

    'rss_admin_unauthorized' => '❌ شما دسترسی به این ربات ندارید. فقط ادمین‌ها می‌توانند استفاده کنند.',
    'rss_admin_bot_not_registered' => '❌ این ربات در سیستم با endpoint webhook-rss-admin ثبت نشده است.',
    'rss_admin_welcome' => '📰 ربات ادمین RSS\n\nاز دکمه‌های زیر استفاده کنید:',
    'rss_admin_btn_add' => '📰 ثبت فید RSS',
    'rss_admin_btn_list' => '📋 لیست فیدها',
    'rss_admin_btn_back' => '◀️ بازگشت',
    'rss_admin_no_tags' => 'هیچ تگی برای کانال‌های RSS تعریف نشده. ابتدا در Nova به Rss Channels تگ اضافه کنید.',
    'rss_admin_choose_tag' => 'تگ مقصد را انتخاب کنید:',
    'rss_admin_feed_list_header' => '📋 فیدهای RSS:',
    'rss_admin_feed_active' => 'فعال',
    'rss_admin_feed_inactive' => 'غیرفعال',
    'rss_admin_no_feeds' => 'هنوز فیدی ثبت نشده است.',
    'rss_admin_send_url' => "تگ «:tag» انتخاب شد.\n\nلینک فید یا @username ویرگول را بفرستید.\nمثال: @samiusblythe",
    'rss_admin_use_buttons' => 'لطفاً از دکمه‌های منو استفاده کنید. /start',
    'rss_admin_duplicate' => 'ℹ️ این فید از قبل ثبت شده (ID: :id).',
    'rss_admin_error_invalid_url' => '❌ لینک نامعتبر است. لینک فید یا @username ویرگول بفرستید.',
    'rss_admin_error_invalid_feed' => '❌ فید RSS معتبر نیست یا قابل دریافت نیست.',
    'rss_admin_error_generic' => '❌ خطا در ثبت فید. دوباره تلاش کنید.',
    'rss_admin_registered' => '✅ فید ثبت شد.\n\nID: :id\nتگ: :tag\n\nخواندن و ارسال خودکار توسط cron انجام می‌شود.',

    'mawkib_finder_welcome' => '🕌 به ربات موکب‌یاب خوش آمدید!',
    'mawkib_finder_ask_phone' => "📱 لطفاً شماره موبایل خود را ارسال کنید:\n\nمی‌توانید دکمه «ارسال شماره موبایل» را بزنید یا شماره را تایپ کنید (مثال: 09123456789).",
    'mawkib_finder_ask_phone_type' => '📱 لطفاً شماره موبایل خود را تایپ کنید (مثال: 09123456789).',
    'mawkib_finder_send_phone_button' => '📱 ارسال شماره موبایل',
    'mawkib_finder_invalid_phone' => '❌ شماره موبایل نامعتبر است. لطفاً دوباره تلاش کنید.',
    'mawkib_finder_bale_only' => '⚠️ ارسال کد OTP فقط از طریق پیام‌رسان بله پشتیبانی می‌شود.',
    'mawkib_finder_ask_otp' => '🔐 کد OTP ارسال‌شده به بله را وارد کنید:',
    'mawkib_finder_otp_sent' => '✅ کد تأیید به بله شما ارسال شد.',
    'mawkib_finder_otp_invalid' => '❌ کد OTP نامعتبر است.',
    'mawkib_finder_otp_expired' => '⏰ کد OTP منقضی شده. لطفاً /start بزنید و دوباره شماره بدهید.',
    'mawkib_finder_otp_not_found' => '⚠️ ابتدا شماره موبایل را ارسال کنید.',
    'mawkib_finder_otp_max_attempts' => '❌ تعداد تلاش‌ها بیش از حد مجاز. لطفاً /start بزنید.',
    'mawkib_finder_otp_send_failed' => '❌ ارسال OTP ناموفق بود. لطفاً بعداً تلاش کنید.',
    'mawkib_finder_otp_rate_limit' => '⏳ محدودیت ارسال OTP. لطفاً کمی بعد تلاش کنید.',
    'mawkib_finder_otp_payment_required' => '❌ موجودی سرویس OTP کافی نیست. با پشتیبانی تماس بگیرید.',
    'mawkib_finder_no_bale_account' => '❌ این شماره در بله حساب ندارد.',
    'mawkib_finder_phone_verified' => '✅ شماره موبایل شما تأیید شد.',
    'mawkib_finder_ask_national_code' => 'لطفاً کد ملی خود را وارد کنید (۱۰ رقم):',
    'mawkib_finder_invalid_national_code' => '❌ کد ملی نامعتبر است. لطفاً کد ملی ۱۰ رقمی صحیح را وارد کنید.',
    'mawkib_finder_identity_not_found' => "❌ این شماره (:mobile) با این کد ملی (:national_code) در سامانه پیشخوان حوزه ثبت نشده است.\n\nلطفاً از صحت شماره موبایل خود در آن سامانه مطمئن شوید و دوباره امتحان کنید.",
    'mawkib_finder_verified_ok' => '✅ شما احراز شدید.',
    'mawkib_finder_select_province' => "روز ورود شما به کدام استان است؟\n\nاستان مورد نظر را انتخاب کنید:",
    'mawkib_finder_results_header' => "📍 نتایج — استان :province:\n",
    'mawkib_finder_city_line' => '🏙 :city: :count جای خالی',
    'mawkib_finder_province_availability_intro' => '📊 در حال حاضر در استان :province شهرهای زیر جای خالی دارند.',
    'mawkib_finder_today_result_hint' => "ℹ️ این نتیجه برای امروز است.\n\nاگر روز ورود شما متفاوت است، تاریخ ورود را انتخاب کنید:",
    'mawkib_finder_select_entry_date' => '📅 تاریخ ورود خود را انتخاب کنید (۱۴ روز — امروز تا ۱۳ روز بعد):',
    'mawkib_finder_today_label' => 'امروز :day :date',
    'mawkib_finder_tomorrow_label' => 'فردا :day :date',
    'mawkib_finder_day_after_label' => 'پس‌فردا :day :date',
    'mawkib_finder_day_label' => ':day :date',
    'mawkib_finder_select_stay_days' => "📅 تاریخ ورود: :date\n\nچند روز در این استان می‌خواهید بمانید؟",
    'mawkib_finder_select_stay_days_prompt' => 'تعداد روز اقامت را انتخاب کنید (۱ تا ۱۴):',
    'mawkib_finder_stay_day_label' => ':days روز',
    'mawkib_finder_confirm_summary' => "📋 خلاصه جستجو:\n\n📅 ورود: :from\n📅 خروج: :to\n🛏 مدت: :days روز\n\nبرای جستجو با این تاریخ‌ها تأیید کنید:",
    'mawkib_finder_confirm_button' => '✅ تأیید و جستجو',
    'mawkib_finder_restart_button' => '🔄 از اول',
    'mawkib_finder_restart_hint' => 'برای شروع مجدد از دکمه زیر استفاده کنید:',
    'mawkib_finder_final_with_dates' => "📍 نتایج — استان :province\n📅 ورود: :from | خروج: :to\n",
    'mawkib_finder_register_urgency' => "🔗 هرچه زودتر در سامانه ثبت‌نام کنید:\n:url\n\n⚠️ تعداد زیادی در حال ثبت‌نام هستند؛ اگر دیر بجنبید ممکن است ظرفیت پر شود.",
    'mawkib_finder_error' => '❌ خطایی رخ داد. لطفاً دوباره /start بزنید.',
    'mawkib_finder_no_availability' => 'ℹ️ در حال حاضر جای خالی در این استان یافت نشد.',
    'mawkib_finder_use_start' => 'لطفاً /start بزنید تا فرآیند را از ابتدا شروع کنید.',

    // MP Contact Bot
    'mp_contact_welcome' => 'به ربات «ارتباط با نماینده مجلس تست» خوش آمدید.
از منوی زیر گزینه مورد نظر را انتخاب کنید.',
    'mp_contact_welcome_admin' => 'پنل مدیریت ربات «ارتباط با نماینده مجلس»
از منوی زیر گزینه مورد نظر را انتخاب کنید.',
    'mp_contact_btn_send_ticket' => '📨 ارسال پیام به نماینده',
    'mp_contact_btn_track_ticket' => '🔎 پیگیری وضعیت تیکت',
    'mp_contact_btn_polls' => '📊 شرکت در نظرسنجی‌های فعال',
    'mp_contact_btn_admin_tickets' => '📬 پیام‌های دریافتی',
    'mp_contact_btn_admin_polls' => '🗳 مدیریت نظرسنجی‌ها',
    'mp_contact_btn_admin_manage' => '👥 مدیریت ادمین‌ها',
    'mp_contact_btn_home' => '🏠 منوی اصلی',
    'mp_contact_btn_back' => '⬅️ بازگشت',
    'mp_contact_btn_next' => 'بعدی ▶️',
    'mp_contact_btn_prev' => '◀️ قبلی',
    'mp_contact_btn_cancel' => '❌ انصراف',
    'mp_contact_btn_agree' => '👍 موافق',
    'mp_contact_btn_disagree' => '👎 مخالف',
    'mp_contact_btn_comment' => '✍️ ثبت نظر توصیفی',
    'mp_contact_btn_create_poll' => '➕ ایجاد نظرسنجی جدید',
    'mp_contact_btn_edit' => '✏️ ویرایش',
    'mp_contact_btn_activate' => '✅ فعال کردن',
    'mp_contact_btn_deactivate' => '⏸ غیرفعال کردن',
    'mp_contact_btn_results' => '📈 نتایج',
    'mp_contact_btn_publish' => '✅ انتشار',
    'mp_contact_btn_approve' => '✅ تأیید',
    'mp_contact_btn_reject' => '❌ رد',
    'mp_contact_btn_yes' => 'بله',
    'mp_contact_btn_no' => 'خیر',
    'mp_contact_ask_ticket' => 'لطفاً پیام متنی خود را برای نماینده ارسال کنید:',
    'mp_contact_ask_tracking_code' => 'کد پیگیری تیکت را وارد کنید:',
    'mp_contact_ask_comment' => 'نظر توصیفی خود را درباره این نظرسنجی بنویسید:',
    'mp_contact_ask_poll_body' => 'متن نظرسنجی را ارسال کنید (مثلاً ۱ یا ۲ خط):',
    'mp_contact_ask_poll_edit' => 'متن جدید نظرسنجی را ارسال کنید:',
    'mp_contact_ticket_created' => '✅ پیام شما ثبت شد.
کد پیگیری: <b>:code</b>
با دکمه «پیگیری وضعیت تیکت» می‌توانید وضعیت را ببینید.',
    'mp_contact_ticket_not_found' => '❌ تیکتی با این کد برای شما یافت نشد.',
    'mp_contact_ticket_status' => '📋 کد: <b>:code</b>
وضعیت: :status
متن: :body',
    'mp_contact_ticket_detail' => '📋 کد: <b>:code</b>
وضعیت: :status
از: :chat_id

:body',
    'mp_contact_ticket_status_updated' => '✅ وضعیت تیکت به‌روز شد.',
    'mp_contact_comment_created' => '✅ نظر شما ثبت شد.
کد رسید: <b>:code</b>',
    'mp_contact_poll_confirm' => 'آیا این نظرسنجی منتشر شود؟

:body',
    'mp_contact_poll_published' => '✅ نظرسنجی منتشر شد.',
    'mp_contact_poll_updated' => '✅ متن نظرسنجی به‌روز شد.',
    'mp_contact_poll_activated' => '✅ نظرسنجی فعال شد.',
    'mp_contact_poll_deactivated' => '⏸ نظرسنجی غیرفعال شد.',
    'mp_contact_poll_unavailable' => '❌ این نظرسنجی در دسترس نیست.',
    'mp_contact_polls_list' => 'نظرسنجی‌های فعال (صفحه :page از :total):',
    'mp_contact_admin_polls_list' => 'مدیریت نظرسنجی‌ها (صفحه :page از :total):',
    'mp_contact_tickets_list' => 'پیام‌های دریافتی (صفحه :page از :total):',
    'mp_contact_no_active_polls' => 'در حال حاضر نظرسنجی فعالی وجود ندارد.',
    'mp_contact_no_tickets' => 'هنوز پیامی ثبت نشده است.',
    'mp_contact_no_polls_admin' => 'هنوز نظرسنجی‌ای ساخته نشده است.',
    'mp_contact_already_voted' => 'شما قبلاً در این نظرسنجی رای داده‌اید.',
    'mp_contact_vote_ok' => '✅ رای شما ثبت شد.',
    'mp_contact_your_vote' => 'رای شما: :choice',
    'mp_contact_poll_results' => '📈 نتایج نظرسنجی

:body

کل شرکت‌کنندگان: :total
موافق: :agree
مخالف: :disagree
نظرات توصیفی: :comments',
    'mp_contact_status_pending' => 'در انتظار',
    'mp_contact_status_reviewing' => 'در حال بررسی',
    'mp_contact_status_closed' => 'بسته شده',
    'mp_contact_use_start' => 'لطفاً /start را بزنید.',
    'mp_contact_not_admin' => 'شما ادمین این ربات نیستید.',
    'mp_contact_already_admin' => 'شما هم‌اکنون ادمین هستید.',
    'mp_contact_admin_request_pending' => 'درخواست ادمین شما در انتظار تأیید است.',
    'mp_contact_admin_request_sent' => '✅ درخواست ادمین شدن شما ارسال شد.',
    'mp_contact_admin_request_notify' => 'درخواست ادمین جدید
Chat ID: :chat_id
شناسه درخواست: #:id',
    'mp_contact_admin_manage_help' => 'مدیریت ادمین‌ها

برای افزودن یا حذف ادمین، پیام یک کاربر را از چت خصوصی به این ربات فوروارد کنید.

درخواست‌های در انتظار را هم می‌توانید از دکمه‌های زیر تأیید/رد کنید.',
    'mp_contact_pending_request_btn' => 'درخواست #:id — :chat_id',
    'mp_contact_primary_only' => 'فقط ادمین اصلی می‌تواند این کار را انجام دهد.',
    'mp_contact_admin_approved' => '✅ درخواست تأیید شد و کاربر ادمین شد.',
    'mp_contact_admin_rejected' => 'درخواست رد شد.',
    'mp_contact_you_are_admin' => '✅ شما به‌عنوان ادمین ربات منصوب شدید. /start را بزنید.',
    'mp_contact_admin_request_rejected_user' => 'درخواست ادمین شدن شما رد شد.',
    'mp_contact_admin_added' => '✅ کاربر به‌عنوان ادمین اضافه شد.',
    'mp_contact_admin_removed' => '✅ دسترسی ادمین حذف شد.',
    'mp_contact_you_removed_admin' => 'دسترسی ادمین شما لغو شد.',
    'mp_contact_cannot_remove_primary' => 'نمی‌توان ادمین اصلی را حذف کرد.',
    'mp_contact_cannot_self' => 'نمی‌توانید پیام خودتان را برای این کار فوروارد کنید.',
    'mp_contact_confirm_add_admin' => 'آیا می‌خواهید این شخص ادمین باشد؟
:name
Chat ID: :chat_id',
    'mp_contact_confirm_remove_admin' => 'این شخص هم‌اکنون ادمین است. آیا می‌خواهید از ادمینی حذف شود؟
:name
Chat ID: :chat_id',
    'channel_poster_not_owner' => 'این ربات فقط برای مالک کانال است.',
    'channel_poster_welcome' => 'به ربات ارسال به کانال‌ها خوش آمدید.',
    'channel_poster_welcome_ready' => 'کانال‌ها آماده‌اند. تگ‌ها: :tags
مطلب بفرست تا به همان تگ ارسال شود. /add برای افزودن کانال یا پلتفرم. /cancel انصراف.',
    'channel_poster_ask_bale_forward' => 'ربات را در کانال بله ادمین کن. بعد یک پست از همان کانال را در خصوصی فوروارد کن. اگر فوروارد کار نکرد، شناسه عددی کانال را بفرست.',
    'channel_poster_need_channel_forward' => 'فوروارد کانال تشخیص داده نشد. یک پست از کانال فوروارد کن، یا شناسه عددی کانال را بفرست (مثلاً با ربات Get Chat ID).',
    'channel_poster_not_admin' => 'نتوانستم در آن کانال پست بگذارم. مطمئن شو ربات ادمین کانال است و دوباره فوروارد کن.',
    'channel_poster_bale_connected' => 'کانال بله وصل شد. یک پست تست آنجا گذاشتم؛ آن را پاک کن.',
    'channel_poster_ask_destination' => 'این مطلب را کجا بفرستم؟',
    'channel_poster_ask_which_tag' => 'این مطلب را برای کدام تگ بفرستم؟',
    'channel_poster_btn_bale' => 'بله',
    'channel_poster_btn_telegram' => 'تلگرام',
    'channel_poster_btn_eitaa' => 'ایتا',
    'channel_poster_btn_soroush' => 'سروش',
    'channel_poster_btn_all' => 'همه',
    'channel_poster_btn_add' => 'افزودن مقصد',
    'channel_poster_btn_new_tag' => 'تگ جدید',
    'channel_poster_coming_soon' => 'تلگرام و ایتا را با /add اضافه کن.',
    'channel_poster_soroush_soon' => 'سروش به‌زودی اضافه می‌شود.',
    'channel_poster_published' => 'مطلب به مقصدهای تگ «:tag» ارسال شد.',
    'channel_poster_publish_failed' => 'ارسال به کانال ناموفق بود. ادمین بودن ربات را بررسی کن.',
    'channel_poster_no_destination' => 'هنوز کانالی وصل نشده. /start بزن و کانال بله را وصل کن.',
    'channel_poster_cancelled' => 'لغو شد. /start یا یک مطلب جدید بفرست.',
    'channel_poster_unsupported' => 'این نوع پیام پشتیبانی نمی‌شود. متن، عکس، صوت یا ویدیو بفرست.',
    'channel_poster_test_message' => 'تست - این پیام را پاک کنید.',
    'channel_poster_empty_text' => '(بدون متن)',
    'channel_poster_no_pending' => 'مطلب معلقی نیست. یک متن، عکس، صوت یا ویدیو بفرست.',
    'channel_poster_untagged' => 'بدون تگ',
    'channel_poster_ask_tag_name' => 'یک نام تگ بفرست (مثلاً شراب بهشتی یا پدر ژپتو). بعداً با همین تگ به بله و تلگرام و ایتا می‌فرستم.',
    'channel_poster_ask_tag_for_existing' => 'این کانال هنوز تگ ندارد: :title
یک نام تگ بفرست (مثلاً شراب بهشتی). اگر مطلب بفرستی همان‌جا ارسال می‌شود. /add برای افزودن تلگرام یا ایتا.',
    'channel_poster_tag_saved' => 'تگ «:tag» ذخیره شد. مطلب بفرست تا به همه مقصدهای این تگ برود. /add برای افزودن تلگرام یا ایتا.',
    'channel_poster_ask_platform' => 'کدام پلتفرم را به تگ «:tag» اضافه کنم؟',
    'channel_poster_ask_new_or_existing_tag' => 'تگ جدید بساز یا به تگ موجود اضافه کن.',
    'channel_poster_ask_telegram_token' => 'توکن ربات تلگرام را بفرست. همان ربات را ادمین کانال تلگرام کن.',
    'channel_poster_ask_telegram_chat_id' => 'شناسه عددی کانال تلگرام را بفرست (مثلاً از Get Chat ID).',
    'channel_poster_ask_eitaa_token' => 'توکن ایتا (ایتایار) را بفرست.',
    'channel_poster_ask_eitaa_chat_id' => 'شناسه کانال ایتا را بفرست.',
    'channel_poster_invalid_token' => 'توکن نامعتبر است. دوباره بفرست یا /cancel.',
    'channel_poster_invalid_chat_id' => 'شناسه نامعتبر است. شناسه عددی کانال را بفرست.',
    'channel_poster_telegram_connected' => 'کانال تلگرام به تگ «:tag» وصل شد.',
    'channel_poster_eitaa_connected' => 'کانال ایتا به تگ «:tag» وصل شد.',

    // === Channel Signature ===
    'channel_poster_sig_bale' => 'در بله',
    'channel_poster_sig_telegram' => 'در تلگرام',
    'channel_poster_sig_eitaa' => 'در ایتا',
    'channel_poster_sig_soroush' => 'در سروش',
    'channel_poster_sign_no_tag' => 'هنوز تگی نداری. اول /start بزن و کانال وصل کن.',
    'channel_poster_sign_ask_tag' => 'لینک کانال کدام تگ را تنظیم کنی؟',
    'channel_poster_sign_pick_channel' => 'کدام کانال را ویرایش کنی؟ (✅ = لینک دارد، ❌ = ندارد)',
    'channel_poster_sign_no_channels' => 'این تگ کانالی ندارد.',
    'channel_poster_sign_current_link' => 'لینک فعلی: :link'."\n".'لینک جدید بفرست یا /cancel.',
    'channel_poster_sign_no_link' => 'لینکی تنظیم نشده. لینک عمومی کانال را بفرست (مثلاً https://eitaa.com/channel_name).',
    'channel_poster_sign_invalid_link' => 'لینک نامعتبر است. لینک باید با http:// یا https:// شروع شود.',
    'channel_poster_sign_link_saved' => 'لینک ذخیره شد: :link'."\n".'با /sign می‌توانی بقیه لینک‌ها را هم تنظیم کنی.',

    // === Publish Options ===
    'channel_poster_ask_publish_options' => 'چطور ارسال شود؟',
    'channel_poster_btn_sig_yes' => '✍️ با امضا',
    'channel_poster_btn_sig_no' => '📄 بدون امضا',
    'channel_poster_btn_now' => '🚀 الان',
    'channel_poster_btn_queue' => '⏰ :time',
    'channel_poster_queue_count' => '(:count مطلب در صف)',

    // === Publish Queue ===
    'channel_poster_queued' => 'در صف قرار گرفت. زمان ارسال: :date ساعت :time',
    'channel_poster_queue_published' => '📢 مطلب از صف منتشر شد:',

    // === Publish Report ===
    'channel_poster_report_header' => '✅ منتشر شد در:',
];
